...
export const PortsPage = () => {
  ...
  const fetchPorts = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail ?? "Error al cargar los puertos.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditDialog = async (port: Port) => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail ?? "Error al cargar el puerto.");
      setDialogOpen(false);
    } finally {
      setLoadingDetail(false);
    }
  ...
  const handleDelete = async (port: Port) => {
    ...
    try {
      setSaving(true);
      await axiosInstance.delete(`/api/ports/${port.id}`);
      await fetchPorts();
    } catch (err: any) {
        ...
        <DialogContent>
          {loadingDetail ? (
            ...
            <Stack spacing={2} mt={1}>
              <TextField label="Nombre" value={currentPort.name ?? ""} onChange={(e) => handleDialogChange("name", e.target.value)} required fullWidth />
              <FormControlLabel
                control={
                  <Checkbox
